...
# X Y Z的最大最小范围  过来人的经验 X Y 尽量贴近 Z尽量大
origin = [-400,-290,0]
extent = [62, 166, 400]
data = pd.read_excel("./surface_points_q模拟示例.xlsx")
...
model.create_and_add_fault(
    "fault1", ##断层名
    40,   ##插值时参与拟合曲面的数据点数量上限
    nelements=1e6, # 指定用于插值的离散网格大小
    interpolator_type="FDI",##分段线性插值 适合断层数据比较稀疏
    # ------------------或者"FDI" 有限充分，通常精度更高但计算更慢
    buffer=0.1,   #控制的是模型空间边界与数据边界之间的扩展比例
    major_axis=465,  #最大轴（长轴）方向上的影响范围，通常平行于断层走向
    minor_axis=150,   #中等轴，控制插值核在断层走向垂直方向的延伸程度
    intermediate_axis=150,#最小轴（厚度方向），通常是断层面垂直方向（厚度越小越尖锐）
)
...
viewer.plot_model_surfaces(cmap="Set3")  #颜色设置tab20    Accent    新配色：gist_earth Set3
...
viewer.plot_block_model(cmap="Accent")
viewer.plot_scalar_field(ground,cmap="Accent")
viewer.show()
