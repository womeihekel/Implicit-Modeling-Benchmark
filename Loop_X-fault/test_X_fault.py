...
model.create_and_add_fault(
    "fault1", ##断层名
    10,   ##插值时参与拟合曲面的数据点数量上限
    nelements=1e5, # 指定用于插值的离散网格大小
    interpolator_type="PLI",##分段线性插值 适合断层数据比较稀疏
    # ------------------或者"FDI" 有限充分，通常精度更高但计算更慢
    buffer=0.1,   #控制的是模型空间边界与数据边界之间的扩展比例
    major_axis=850,  #最大轴（长轴）方向上的影响范围，通常平行于断层走向
    minor_axis=200,   #中等轴，控制插值核在断层走向垂直方向的延伸程度
